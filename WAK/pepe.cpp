/**
 * Wykorzystano fragmenty kodu algorytmu wegierskiego
 * ze strony http://www.topcoder.com/tc?module=Static&d1=tutorials&d2=hungarianAlgorithm
 * autorstwa x-ray'a
 */
...
using namespace std;
#define N 5000             //max number of vertices in one part
...
main() {
	...
	REP(i,k) REP(j,n) {
		...
		a = tab[a];
		REP(l,n) {
			cost[a-1][l] += min(8, abs(l-j));
		}
	}
	REP(i,n) REP(j,n) cost[i][j] = 24 - cost[i][j];
	printf("%d\n", n*24 - hungarian());
}
